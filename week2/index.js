...
var l = '8';
// 請文字解釋為什麼
console.log(k * l > 21);//true 在乘法運算時，會把字串轉為數字
...
if (BobIsVip === true || BobPrice >= 1599) {
...
if(coachIncome <= 100000){
  ...
  coachBonus = coachIncome* 0.2 + baseBonus;
}
...
if (performanceData.coaches[0].performance < 50000) {
  // 請填寫第一位教練業績增長程式碼，使用 +=
  performanceData.coaches[0].performance += (50000-42000);
}
...
if (performanceData.coaches[1].performance < 50000) {
  // 請填寫第二位教練業績增長程式碼，使用 +=
  performanceData.coaches[1].performance += (50000-38000);
}
...
let calorieBurn = 0;
let bikeMin = 20;
let runningMachineMin = 30;
let yogaMin = 40

// 練習：計算小明今日消耗的卡路里
calorieBurn = (activities['騎腳踏車'].minBurnKar*bikeMin)+(activities['跑步機'].minBurnKar*runningMachineMin)+(activities['瑜伽'].minBurnKar*yogaMin)
...
if (exerciseRecords[0].duration >= 30 && exerciseRecords[0].heartRate >= 130) {
  ...
  validDays += 1;
  console.log(validDays,totalDuration);
}
// 練習：週二、週三、週四、週五、週六
